...
int manhattanIntersection(vector<Segment> S) {
    ...
    for (int i = 0, k = 0; i < n; i++) {
        if (S[i].v1.y == S[i].v2.y) {
            if (S[i].v1.x > S[i].v2.x) swap(S[i].v1, S[i].v2);
        } else if (S[i].v1.y > S[i].v2.y) swap(S[i].v1, S[i].v2);

        if ( S[i].v1.y == S[i].v2.y) {
            EP[k++] = EndPoint(S[i].v1, i, LEFT);
            EP[k++] = EndPoint(S[i].v2, i, RIGHT);
        } else {
            EP[k++] = EndPoint(S[i].v1, i, BOTTOM);
            EP[k++] = EndPoint(S[i].v2, i, TOP);
        }
    }
    ...
    for (int i = 0; i < 2 * n; i++) {
        if (EP[i].st == TOP) {
            ...
            auto b = BT.lower_bound(S[EP[i].seg].v1.x);
            auto e = BT.upper_bound(S[EP[i].seg].v2.x);
            cnt += distance(b, e);
        }
    }
...
int main() {
    ...
    REP(i, n) {
        ...
        G.push_back(Segment(Point(x1, y1), Point(x2, y2)));
    }

    int ans = manhattanIntersection(G);
